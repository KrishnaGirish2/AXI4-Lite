...
always @(*) begin
$display("TIME=%0t STATE=%0d aw_done=%0d w_done=%0d AWV=%0d AWR=%0d WV=%0d WR=%0d",
         $time, wr_state, aw_done, w_done,
...
//$display("time %0t wr_state=%d AWVALID =%b AWREADY=%b WVALID=%b WREADY=%b BVALID=%b BREADY=%b",$time, wr_state,AWVALID, AWREADY, WVALID, WREADY, BVALID, BREADY);
        case (wr_state)
            W_IDLE: begin
//$display("W_Idle  AWVALID =%b AWREADY=%b WVALID=%b WREADY=%b BVALID=%b BREADY=%b",aw_done,AWVALID, AWREADY, WVALID, WREADY, BVALID, BREADY);
            ...
            W_BOTH: begin
//$display("W_Both awdone=%d w_done=%d  AWVALID =%b AWREADY=%b WVALID=%b WREADY=%b BVALID=%b BREADY=%b",w_done, aw_done,AWVALID, AWREADY, WVALID, WREADY, BVALID, BREADY);
            ...
            W_RESP: begin
$display("W_RESP DEBUG: AWADDR=%h wr_addr_lat=%h",
         AWADDR, wr_addr_lat);
                if (!BVALID) begin
                    if (!wr_addr_ok) begin
                        BRESP  <= RESP_DECERR;
